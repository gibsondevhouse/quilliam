 ...
 *
 * Streams the Ollama response as-is (NDJSON, Content-Type: text/event-stream).
 * Canonical entity extraction happens client-side after the stream completes
 * via `buildPatchesFromExtraction` in `useLocalChat` — no second Ollama call.
 * Extraction is gated on `executionMode !== "local"` in Chat/index.tsx.
 */
